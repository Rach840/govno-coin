...
export const user_reg = async (req: Request, res: Response) => {
  const { user_id, username, user_age, user_height, user_weight, user_sex, user_toilet_visits } = req.body
  let { referredCode } = req.body

  if (!user_id || !user_age || !user_height || !user_weight || !user_toilet_visits) {
  ...
  try {
    await db.tx(async (t) => {
      ...
      const existingUser = await t.oneOrNone(`SELECT ref_code FROM govno_db.users WHERE tg_user_hash = $1`, [hashedId])

      let userRefCode = existingUser ? existingUser.ref_code : generateRefCode()
      ...
      if (!existingUser) {
        ...
        await t.none(
          `INSERT INTO govno_db.user_properties (tg_user_hash, user_age, user_height, user_weight, user_sex, user_toilet_visits)
           VALUES ($1, $2, $3, $4, $5, $6)`,
          [hashedId, user_age, user_height, user_weight, user_sex, user_toilet_visits]
        )
      ...
      if (referredCode && referredCode !== userRefCode) {
        const referredUser = await t.oneOrNone(`SELECT tg_user_hash, username FROM govno_db.users WHERE ref_code = $1`, [referredCode])
        ...
        if (referredUser) {
          ...
          console.log(`Реферал засчитан от ${referredUser.username ? '@' + referredUser.username : 'неизвестного'}`)
        } else {
...
async function updateLoginStreak(user_id: number, t: any) {
  ...
  const lastLogin = user.last_login_date ? DateTime.fromISO(user.last_login_date).toISODate() : null
  ...
  console.log(`✅ Login streak обновлён для ${user_id}: ${newStreak} дней подряд`)
}
