...
async function callOpenAIWithRetry(payload: OpenAI.Chat.ChatCompletionCreateParamsNonStreaming, maxRetries = 3, delayMs = 1000): Promise<OpenAI.Chat.ChatCompletion> {
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if ('choices' in response) {
        ...
        throw new Error('Получен потоковый ответ вместо обычного. Ожидался обычный ответ.')
      }
    } catch (error: any) {
  ...
  throw new Error('Не удалось получить ответ от OpenAI после нескольких попыток')
}
...
export const generateQuestions = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const user = await db.oneOrNone(`SELECT last_quiz_generate_time, user_banned FROM govno_db.users WHERE tg_user_hash = $1`, [tg_hash])
    ...
    if (user.last_quiz_generate_time) {
      ...
      if (diff < 24) {
        res.status(403).json({ error: 'Вопросы можно генерировать раз в сутки' })
        return
      }
    }
    ...
    await db.none(`UPDATE govno_db.users SET last_quiz_generate_time = NOW() WHERE tg_user_hash = $1 AND user_banned = false`, [tg_hash])
...
export const validateAnswers = async (req: Request, res: Response) => {
  ...
  try {
    await db.tx(async (t) => {
      ...
      await t.none(`UPDATE govno_db.users SET last_quiz_answer_time = NOW() WHERE tg_user_hash = $1`, [hashTgId(user_id)])
      ...
      const completion = await callOpenAIWithRetry({
        ...
        messages: [
          {
            role: 'system',
            content: 'Ты помощник, который оценивает ответы на вопросы. Ответь массивом из true/false без лишнего текста. true — если ответ правильный, false — если нет.',
          },
          {
            role: 'user',
      ...
      const cleanedText = validationText.replace(/[^\[\]{}:,"0-9a-zA-Z\s]/g, '').trim()
      ...
      try {
        ...
        if (!Array.isArray(validationResults)) {
          res.status(400).json({ error: 'Некорректный формат валидации от AI' })
      ...
      const correctAnswersCount = validationResults.filter((v) => v === true).length
      const reward = Number((correctAnswersCount * 0.01).toFixed(6))

      console.log(`Правильных ответов: ${correctAnswersCount}, награда: ${reward}`)
